...
let selfCitationFound = false;
let refCitationDataFound = false;
let result = false;
        ...
        (Object.keys(wordCounts).length === 0) 
          ...
          : Object.keys(wordCounts)
            .map((key) => {
              ...
              return `<div style='color: ${wordMatchColor[key] || "black"};'>${key}: ${count}</div>`;
            })
            .join("")
      }
    </div>
  ...
  ${
    highlightedText = Object.keys(abbv_list).map(key => {
      if (abbv_list[key]) {
        return `<div key="${key}">
          ${key} - ${abbv_list[key]}
        </div>`;
...
app.post("/api/upload", upload.single("file"), (req, res) => {
  ...
  extracted
    .then(async function (doc) {
      ...
      const {abbvHighlightedText, abbv_list} =  abbreviation(extractedText)




console.log("1 - ***********", abbvHighlightedText);
console.log("2 - ***********",abbv_list);
